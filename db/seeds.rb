...
loop do
  ...
  html_doc.search(".product-name>a").each do |element|
    ...
    # Surfboard.create(url: url, name: name, level: level, volume: volume, dimensions: dimensions, price: price, image: image)
    sleep(0.5)
  end
   if i == 5
     break
...
loop do
  ...
  html_doc.search(".product-name>a").each do |element|
    ...
    # Wetsuits.create(url: url, name: name, thickness: thickness, zip: zip, price: price, image: image)
    sleep(0.5)
  end
    if i == 5
      break
